...
class TestSimple(unittest.TestCase):
    ...
    def testLineEnds(self):
        ...
        for lineEndType in [self.ed.SC_EOL_CR, self.ed.SC_EOL_LF, self.ed.SC_EOL_CRLF]:
            ...
            self.assertEquals(self.ed.Contents(), b"x" + lineEnds[
                              lineEndType] + b"y")
            self.assertEquals(self.ed.LineLength(
                0), 1 + len(lineEnds[lineEndType]))
    ...
    def testUnicodeLineEndsWithCodePage0(self):
        # Try the Unicode line ends when not in Unicode mode -> should remain 1
        # line
    ...
    def testUBreakApartEOL(self):
        ...
        # only one line after each removal of any byte in line end and 2 lines
        # after reinsertion
        self.ed.Lexer = self.ed.SCLEX_CPP
        self.ed.SetCodePage(65001)
        self.ed.SetLineEndTypesAllowed(1)
    ...
    def testURemoveEOLFragment(self):
        # Add UTF-8 line end then delete each byte causing line end to
        # disappear
        self.ed.Lexer = self.ed.SCLEX_CPP
        self.ed.SetCodePage(65001)
        self.ed.SetLineEndTypesAllowed(1)
...
class TestIndicators(unittest.TestCase):
    ...
    def testIndicatorAtEnd(self):
        ...
        # Now only one character left and does not have indicator so indicator
        # 3 is null
        self.assertEquals(self.ed.IndicatorValueAt(3, 0), 0)
...
class TestSearch(unittest.TestCase):
    ...
    def testREFind(self):
        ...
        self.assertEquals(2, self.ed.FindBytes(
            0, self.ed.Length, b"b.g", flags))
        self.assertEquals(2, self.ed.FindBytes(
            0, self.ed.Length, b"\<b.g\>", flags))
        self.assertEquals(-1, self.ed.FindBytes(0, self.ed.Length, b"b[A-Z]g",
                                                flags | self.ed.SCFIND_MATCHCASE))
        self.assertEquals(2, self.ed.FindBytes(
            0, self.ed.Length, b"b[a-z]g", flags))
        self.assertEquals(6, self.ed.FindBytes(
            0, self.ed.Length, b"b[a-z]*t", flags))
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"^a", flags))
        self.assertEquals(10, self.ed.FindBytes(
            0, self.ed.Length, b"\t$", flags))
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"\([a]\).*\0", flags))
    ...
    def testPhilippeREFind(self):
        ...
        flags = self.ed.SCFIND_REGEXP
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"\w", flags))
        self.assertEquals(1, self.ed.FindBytes(
            0, self.ed.Length, b"\W", flags))
        self.assertEquals(-1, self.ed.FindBytes(
            0, self.ed.Length, b"\d", flags))
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"\D", flags))
        self.assertEquals(1, self.ed.FindBytes(
            0, self.ed.Length, b"\s", flags))
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"\S", flags))
        self.assertEquals(2, self.ed.FindBytes(
            0, self.ed.Length, b"\x62", flags))

    def testRENonASCII(self):
        self.ed.InsertText(0, b"\xAD")
        flags = self.ed.SCFIND_REGEXP
        self.assertEquals(-1, self.ed.FindBytes(
            0, self.ed.Length, b"\\x10", flags))
        self.assertEquals(2, self.ed.FindBytes(
            0, self.ed.Length, b"\\x09", flags))
        self.assertEquals(-1, self.ed.FindBytes(
            0, self.ed.Length, b"\\xAB", flags))
        self.assertEquals(0, self.ed.FindBytes(
            0, self.ed.Length, b"\\xAD", flags))
...
class TestMultiSelection(unittest.TestCase):
    ...
    def testRectangularVirtualSpaceOptionOff(self):
        # Same as previous test but virtual space option off so no virtual
        # space in result
...
class TestCaseMapping(unittest.TestCase):
    ...
    def tearDown(self):
        self.ed.SetCodePage(0)
        self.ed.StyleSetCharacterSet(
            self.ed.STYLE_DEFAULT, self.ed.SC_CHARSET_DEFAULT)
    ...
    def testRussian(self):
        if sys.platform == "win32":
            self.ed.StyleSetCharacterSet(
                self.ed.STYLE_DEFAULT, self.ed.SC_CHARSET_RUSSIAN)
        else:
            self.ed.StyleSetCharacterSet(
                self.ed.STYLE_DEFAULT, self.ed.SC_CHARSET_CYRILLIC)
    ...
    def testUTFGrows(self):
        # This crashed at one point in debug builds due to looking past end of
        # shorter string
        self.ed.SetCodePage(65001)
