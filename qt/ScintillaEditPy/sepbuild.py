...
# Decide up front which platform, treat anything other than Windows or OS
# X as Linux
...
def textFromRun(args):
    (stdoutdata, stderrdata) = subprocess.Popen(args,
...
def usage():
    ...
    print(
        "-u --underscore-names  use method_names consistent with GTK+ standards")
        ...
        if v["Category"] != "Deprecated":
            ...
            if feat in ["fun", "get", "set"]:
                ...
                if checks:
                    ...
                    out.append(modifyFunctionElement % (
                        methodSignature(name, v, options), inject))
                # if v["Param1Type"] == "string":
...
class SepBuilder:
    def __init__(self):
        ...
        self.ScintillaEditIncludes = [
            ".", "../ScintillaEdit", "../ScintillaEditBase", "../../include"]
        if PLAT_WINDOWS:
        ...
        if not PLAT_WINDOWS:
            ...
            pathToQMake = textFromRun(
                "which qmake-qt4 || which qmake").rstrip()
            self.QMakeCommand = os.path.basename(pathToQMake)

        # Qt default location from qmake
        self._SetQtIncludeBase(textFromRun(
            self.QMakeCommand + " -query QT_INSTALL_HEADERS").rstrip())
    ...
    def _setPySideBase(self, base):
        ...
        if PLAT_LINUX:
            self.PySideTypeSystem = textFromRun(
                "pkg-config --variable=typesystemdir pyside").rstrip()
            self.PySideIncludeBase = textFromRun(
                "pkg-config --variable=includedir pyside").rstrip()
            self.ShibokenIncludeBase = textFromRun(
                "pkg-config --variable=includedir shiboken").rstrip()
        else:
            self.PySideTypeSystem = os.path.join(
                self.PySideBase, "share", "PySide", "typesystems")
            self.ShibokenIncludeBase = os.path.join(
                self.PySideBase, "include", "shiboken")
            self.PySideIncludeBase = os.path.join(
                self.PySideBase, "include", "PySide")
        ...
        self.AllIncludes = os.pathsep.join(
            self.QtIncludes + self.ScintillaEditIncludes + self.PySideIncludes)
        ...
        # self.ShibokenGenerator = os.path.join(self.PySideLibDir,
    ...
    def generateAPI(self, args):
        ...
        GenerateFile(
            "typesystem_ScintillaEdit.xml.template", "typesystem_ScintillaEdit.xml",
    ...
    def writeVariables(self):
        # Write variables needed into file to be included from project so it
        # does not have to discover much
        with open(self.ProInclude, "w") as f:
            ...
            f.write("PYSIDE_INCLUDES=" + doubleBackSlashes(
                self.PySideIncludeBase) + "\n")
            f.write("PYSIDE_LIB=" + doubleBackSlashes(
                self.PySideLibDir) + "\n")
            f.write("SHIBOKEN_INCLUDES=" + doubleBackSlashes(
                self.ShibokenIncludeBase) + "\n")
            if self.DebugBuild:
    ...
    def buildEverything(self):
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                self._SetQtIncludeBase(os.path.join(
                    os.path.normpath(arg), 'include'))
    ...
    def _SetQtIncludeBase(self, base):
        ...
        self.QtIncludes = [self.QtIncludeBase] + [os.path.join(
            self.QtIncludeBase, sub) for sub in ["QtCore", "QtGui"]]
