...
def underscoreName(s):
    ...
    irregular = ['WS', 'EOL', 'AutoC', 'KeyWords',
                 'BackSpace', 'UnIndents', 'RE', 'RGBA']
                ...
                if checkTypes(name, v):
                    ...
                    if stringResult:
                        returns += "    " + returnStatement + \
                            "TextReturner(" + featureDefineName + ", "
...
def usage():
    ...
    print("")
    print(
        "Generate full APIs for ScintillaEdit class and ScintillaConstants.py.")
    print("")
    print("options:")
    print("")
...
def main(argv):
    ...
    opts, args = getopt.getopt(argv, "hcgu", [
                               "help", "clean", "gtk-check", "underscore-names"])
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            usage()
            sys.exit()
